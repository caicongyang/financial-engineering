...
class StockTrendsSSEClient:
    ...
    async def get_active_stocks(self, min_count=2):
        """
        获取活跃股票列表
        :param min_count: 最小活跃次数，默认为2表示至少两次成交��大于5万
    ...
    async def process_sse_data(self, data):
        """处理 SSE 数据"""
        try:
            logger.debug(f"Raw SSE data received: {data[:200]}...")  # 只打印前200个字符避免日志过长
            
            if data.startswith('data:'):
                data = data[5:].strip()
                logger.debug(f"Stripped data: {data[:200]}...")
            
            if data:
                try:
                    json_data = json.loads(data)
                    logger.debug(f"Parsed JSON data: {json.dumps(json_data, ensure_ascii=False)[:200]}...")
                    ...
                    if (json_data.get('data') and 
                        ...
                        logger.info(f"Processing trends data for stock {stock_code}, trends count: {len(trends)}")
                        
                        await self.save_trends_data(stock_code, trends)
                    else:
                        logger.warning(f"Invalid data structure: {json.dumps(json_data, ensure_ascii=False)[:200]}...")
                        
                except json.JSONDecodeError as je:
                    logger.error(f"JSON decode error: {je}")
                    logger.error(f"Problematic data: {data[:500]}")  # 打印更多内容以便调试
                            ...
                            try:
                                async for chunk in response.content.iter_chunked(1024 * 1024):
                                    try:
                                        ...
                                        logger.debug(f"Received chunk for {stock_code}: {chunk_text[:200]}...")
                                        buffer += chunk_text
                                        ...
                                        while '}' in buffer:
                                            try:
                                                ...
                                                if json_str:
                                                    logger.debug(f"Processing JSON string: {json_str[:200]}...")
                                                    self.last_data_time = datetime.now()
                                                    await self.process_sse_data(json_str)
                                            except ValueError as ve:
                                                logger.warning(f"Incomplete JSON data for {stock_code}: {ve}")
                                                break
                                                
                                    except UnicodeDecodeError as ude:
                                        logger.error(f"Unicode decode error for stock {stock_code}: {ude}")
                                        logger.error(f"Problematic chunk: {chunk[:100]}")
                                        buffer = ""
                                        continue
                                        
                            except aiohttp.ClientPayloadError as cpe:
                                logger.error(f"Payload error during read for stock {stock_code}: {cpe}")
                                await asyncio.sleep(5)
                                break
