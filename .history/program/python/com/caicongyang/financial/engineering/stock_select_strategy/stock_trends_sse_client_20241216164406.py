                ...
                if len(fields) == 8:
                    ...
                    if volume > 50000:  # 成交量大于5万
                        ...
                        if current_count is None:
                            # 第一次活跃
                            pipe.hset(active_stocks_key, stock_code, 1)
                            logger.info(f"Stock {stock_code} first active with volume {volume}")
                        else:
                            # 已经有活跃记录，增加计数
                            new_count = int(current_count) + 1
                            pipe.hset(active_stocks_key, stock_code, new_count)
                            logger.info(f"Stock {stock_code} active count increased to {new_count} with volume {volume}")
